...
def ask_followup(summary, question):
    prompt = f"Based on the following summary:\n\n{summary}\n\n .Please answer the following Question: {question} and please do not show any internal reasoning, chain-of-thought, or explanation of your process. Answer it in detailed and simple words without excessive jargons."
    response = client.chat.completions.create(
        model="deepseek/deepseek-r1:free",
        messages=[
...
st.markdown("Upload any PDF to summarize it in simple words and explain it.")

uploaded_file = st.file_uploader("", type=["pdf"],key="general")
if uploaded_file:
    text = extract_text(uploaded_file)
    if text:
        with st.spinner("Hol' up! Let us Cook... 🧑‍🍳"):
            summary=summarize_text(text, "general") 
